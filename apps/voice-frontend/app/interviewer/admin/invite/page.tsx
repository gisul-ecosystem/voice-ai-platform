...
export default function InviteCandidatesPage() {
  ...
  function update(index: number, patch: Partial<Candidate>) { setCandidates((items) => items.map((item, itemIndex) => itemIndex === index ? { ...item, ...patch } : item)); }
  function addCandidate() { setCandidates((items) => [...items, { name: "", email: "", resume: null }]); }
  async function invite(index: number) {
    const candidate = candidates[index];
    if (!candidate.name.trim() || !candidate.email.trim() || !candidate.resume || !state || !draft) { update(index, { error: "Name, email, and CV are required." }); return; }
    update(index, { busy: true, error: undefined });
    try {
      const create = await fetch("/api/admin/candidates", { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ name: candidate.name, email: candidate.email }) });
      const created = await create.json().catch(() => ({}));
      if (!create.ok) throw new Error(String(created.error || "Candidate creation failed."));
      const form = new FormData(); form.append("file", candidate.resume);
  ...
  return <main className="interviewer-home admin-builder-page">
    ...
    <section className="demo-intro"><p className="eyebrow">Published interview</p><h1>Invite candidates</h1><p>Use <strong>{definitionId}</strong> for every candidate. Each gets an independent CV, session, transcript, and evaluation.</p></section>
    <section className="demo-card invite-page-card">
      <div className="pipeline-panel"><div><span className="section-number">PIPELINE</span><strong>Provider readiness</strong></div><div className="pipeline-services">{pipeline?.services.map((service) => <span className={service.configured ? "pipeline-chip is-ready" : "pipeline-chip"} key={service.name}><i />{service.name} · {service.provider}</span>)}<span className={pipeline?.backend ? "pipeline-chip is-ready" : "pipeline-chip"}><i />Backend</span></div></div>
      {error ? <div className="alert" role="alert">{error}</div> : null}
      {candidates.map((candidate, index) => <fieldset className="candidate-invite-card" key={index}><legend>Candidate {index + 1}</legend><div className="admin-field-grid"><label>Name<input value={candidate.name} onChange={(e) => update(index, { name: e.target.value })} /></label><label>Email<input type="email" value={candidate.email} onChange={(e) => update(index, { email: e.target.value })} /></label><label>CV<input type="file" accept=".pdf,.docx,.txt,.md" onChange={(e) => update(index, { resume: e.target.files?.[0] || null })} /></label></div>{candidate.error ? <p className="candidate-error">{candidate.error}</p> : null}{candidate.invite ? <div className="invitation-link"><label>Secure invite link</label><input readOnly value={`${window.location.origin}${candidate.invite}`} /></div> : <button className="button primary" type="button" disabled={candidate.busy} onClick={() => void invite(index)}>{candidate.busy ? "Creating invite..." : "Create candidate invite"}</button>}</fieldset>)}
      <button className="button secondary add-candidate-button" type="button" onClick={addCandidate}>+ Add another candidate</button>
    </section>
  </main>;
}
